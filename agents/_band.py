...
def _load_band_messaging_rules() -> str:
    rules_path = ROOT / "prompts" / "band_messaging_rules.md"
    if rules_path.exists():
        return rules_path.read_text(encoding="utf-8")
    return ""


def build_adapter(agent_name: str, prompt: str) -> AimlOpenAIAdapter:
    """Build AI/ML API adapter with execution reporting enabled."""
    api_key = os.getenv("AIML_API_KEY")
    if not api_key:
        raise ValueError("AIML_API_KEY is not configured.")
    full_prompt = prompt
    rules = _load_band_messaging_rules()
    if rules:
...
def create_band_agent(agent_name: str, prompt: str) -> Agent:
    ...
    return Agent.create(
        ...
        adapter=build_adapter(agent_name, prompt),
