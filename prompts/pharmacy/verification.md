...
Post **one** band_send_message with your JSON verdict only. Do not post a second summary message.

The platform converts your JSON into the formatted VERIFICATION COMPLETE (or CAUTION / ESCALATION) message automatically.

**Never post:**
- Messages starting with `---`
- Plain English summaries labeled "SUMMARY FOR HUMAN REVIEW"
- Legacy templates like "Verification passed" or "Passing to Resource check now"

Use `requested_service` from the intake payload in your JSON `drug_name` field when available.
